...
from qt_design.python_design.authorization_design import Ui_Authorization_Design
from qt_design.python_design.check_code_design import Ui_Check_Code_Design
        ...
        if result:
            if password == result[4]:
                ...
                if result[5] == 'Читатель':
                    ...
                    self.main_window.create_piechart_1(result[1])
...
class MyMainWindow_Dev(QMainWindow, Ui_MainWindow_Design_Dev):
    def __init__(self, parent=Authorization):
        ...
        self.check_code = CheckCode(self)
        ...
        self.pushButton_9.clicked.connect(self.check_delete_all_books)
    ...
    year       INT     NOT NULL,
    publisher  TEXT    NOT NULL ON CONFLICT IGNORE,
    translator TEXT,
    genre      TEXT    NOT NULL ON CONFLICT IGNORE
)"""
        cur.execute(request)
        # сохраняем изменения
        connection.commit()

    def check_delete_all_books(self):
        self.check_code.show()

    def delete_all_book(self):
        try:
            cur = self.con.cursor()
            request = """DROP TABLE IF EXISTS books"""
            cur.execute(request)
            connection.commit()
        except:
...
class MyMainWindow_User(QMainWindow, Ui_MainWindow_Design_User):
    ...
    def create_piechart_1(self, name):
        req = """SELECT list_books FROM bookmarks WHERE name = ?"""
        cur = connection.cursor()
        result = cur.execute(req, (name,)).fetchall()
        help = []
        b = False
        if len(result) > 0:
            help = [i for i in result[0]]
            ...
            if help[0] != '':
                ...
                for i in help[0].split(','):
                    i = int(i)
                    req = """SELECT genre FROM books WHERE id = ?"""
                    result = cur.execute(req, (i,)).fetchone()
                    result_genre.append(result[0])
                result_genre_2 = {}
                for i in result_genre:
                    if i not in result_genre_2:
                        result_genre_2[i] = result_genre.count(i)
                ...
                if len(key) < 5:
                    for i in range(len(key)):
                        series.append(key[i], result_genre_2[key[i]])
                else:
                    series.append(key[0], result_genre_2[key[0]])
                    series.append(key[1], result_genre_2[key[1]])
                    series.append(key[2], result_genre_2[key[2]])
...
class CheckCode(QMainWindow, Ui_Check_Code_Design):
    def __init__(self, parent=None):
        super(CheckCode, self).__init__(parent)
        self.setupUi(self)
        self.pushButton_2.clicked.connect(self.save_check_code)

    def save_check_code(self):
        code = self.lineEdit_2.text()
        if code == '456':
            valid = QMessageBox.question(
                self, 'Подтверждение действий', "Вы действительно хотите удалить таблицу с книгами?",
                QMessageBox.Yes, QMessageBox.No)
            if valid == QMessageBox.Yes:
                self.parent().delete_all_books()
            self.close()
        else:
            valid = QMessageBox.warning(
                self, 'Ошибка доступа', "Неправильный код библиотеки!")
